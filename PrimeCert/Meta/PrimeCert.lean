...
**Deprecated**: use the `prime_cert` tactic instead. Retained for backwards compatibility;
each use emits a deprecation warning.
...
elab "prime_cert% " "[" grps:step_group,+ "]" : term => do
  logWarning "`prime_cert%` is deprecated; use the `prime_cert` tactic instead"

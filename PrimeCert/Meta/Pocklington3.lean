...
an explicit `m` still parses but is **deprecated**.
...
/-- Legacy `pock3` step with an explicit sieve bound `m` (deprecated; `m` is now computed). -/
...
def parsePock3Spec : PrimeCertMethod `pock3_spec := fun stx dict ↦ do
  ...
  let m ← match mOpt with
    | some mStx => do
      logWarning "the `m` argument to `pock3` is deprecated and now computed automatically; \
        use `pock3 (N, root, mode, F)`"
      pure mStx.getNat
    | none => do
